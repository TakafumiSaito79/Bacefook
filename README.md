...
- 4/12(金) のブロック 2 で行われるプレゼンテーションの準備をする
- 以下の内容を含んで発表を行なってください。1 チームあたりの時間は後ほど伝えます。
  - 完成したソーシャルメディアアプリの紹介
...
投稿（post）オブジェクトが作成されると、投稿の内容（オブジェクト）が `bacefook.newsfeed` の配列と friend の配列にそれぞれ追加されます。したがって、Hanako が '作成' した投稿（post）オブジェクトは、あなたの Bacefook と `bacefook.friends.hanako` の配列にそれぞれ追加されます。
